...
public partial class Admin_create_role : System.Web.UI.Page
{
            ...
            if (Page.IsValid)
            {
                ...
                if (btnSave.Text == "Update")
                {
                    if (CreateUser.CheckAccess(ConDYN, pageName, "Edit", Request.Cookies["dyn_aid"].Value))
                    {
                        ...
                        ca.Status = "Active";
                        int result = UserRoles.UpdateUserRoles(ConDYN, ca);
                        if (result > 0)
                        {
                            GetAllRoles();
                            GetRoles();
                ...
                {
                    if (CreateUser.CheckAccess(ConDYN, pageName, "Add", Request.Cookies["dyn_aid"].Value))
                    {
                        ca.RoleName = txtName.Text;
                        ca.UpdatedOn = TimeStamps.UTCTime();
                        ca.Status = "Active";
                        int result = UserRoles.InsertUserRoles(ConDYN, ca);
                        if (result > 0)
                        {
                            txtName.Text = "";
                            GetAllRoles();
    ...
    public void GetAllRoles()
    {
        ...
        {
            strUserRoless = "";
            List<UserRoles> cas = UserRoles.GetAllUserRoles(ConDYN).OrderByDescending(s => Convert.ToDateTime(s.UpdatedOn)).ToList();
            int i = 0;
...
<td>" + ca.UpdatedOn.ToString("dd-MMM-yyyy") + @"</td>  
    ...
    public static string Delete(string id)
    {
        ...
        {
            SqlConnection ConDYN = new SqlConnection(ConfigurationManager.ConnectionStrings["ConDYN"].ConnectionString);
            if (CreateUser.CheckAccess(ConDYN, "create-role.aspx", "Delete", HttpContext.Current.Request.Cookies["dyn_aid"].Value))
            {
                UserRoles cat = new UserRoles();
                cat.Id = Convert.ToInt32(id);
                cat.UpdatedOn = TimeStamps.UTCTime();
                int exec = UserRoles.DeleteUserRoles(ConDYN, cat);
                if (exec > 0)
                {
                    x = "Success";
                }
