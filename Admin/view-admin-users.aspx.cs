...
public partial class Admin_view_admin_users : System.Web.UI.Page
{
    ...
    public void GetAllUsers()
    {
        ...
        {
            strUsers = "";
            List<CreateUser> instrs = CreateUser.GetAllUser(ConDYN);
            int i = 0;
...
<div class='form-check form-switch markBlocked' id='chk_" + cat.Id + @"'  data-id='" + cat.Id + @"'>
  ...
  <td>" + chk + @"</td> 
    ...
    public static string Delete(string id)
    {
        ...
        {
            SqlConnection ConDYN = new SqlConnection(ConfigurationManager.ConnectionStrings["ConDYN"].ConnectionString);
            if (CreateUser.CheckAccess(ConDYN, "view-admin-users.aspx", "Delete", HttpContext.Current.Request.Cookies["dyn_aid"].Value))
            {
                ...
                cat.AddedBy = HttpContext.Current.Request.Cookies["dyn_aid"].Value;
                int exec = CreateUser.DeleteUser(ConDYN, cat);
                if (exec > 0)
                {
                    x = "Success";
                }
    ...
    public static string BlockUsers(string id, string ftr)
    {
        ...
        {
            SqlConnection ConDYN = new SqlConnection(ConfigurationManager.ConnectionStrings["ConDYN"].ConnectionString);
            if (CreateUser.CheckAccess(ConDYN, "view-admin-users.aspx", "Edit", HttpContext.Current.Request.Cookies["dyn_aid"].Value))
            {
                int exec = 0;
                CreateUser pro = new CreateUser();
                if (ftr == "Yes")
                {
                    exec = CreateUser.BlockUser(ConDYN, id, HttpContext.Current.Request.Cookies["dyn_aid"].Value);
                    vali = "Blocked";
                }
                else
                {
                    exec = CreateUser.UnBlockUser(ConDYN, id, HttpContext.Current.Request.Cookies["dyn_aid"].Value);
                    vali = "UnBlocked";
                }
