...
{
    SqlConnection ConDYN = new SqlConnection(ConfigurationManager.ConnectionStrings["ConDYN"].ToString());
    ...
    public void CheckPassKeyChanged()
    {
        try
        {
            ...
            {
                string stored_pass_key = Request.Cookies["dyn_apkv"].Value;
                var udetails = CreateUser.GetUserDetails(ConDYN, Request.Cookies["dyn_aid"].Value);
                if (udetails != null)
                {
    ...
    protected void btnLock_Click(object sender, EventArgs e)
    {
        try
        {
            if (Request.Cookies["dyn_aid"] != null)
            {
                string uid = Request.Cookies["dyn_aid"].Value;
                Response.Cookies["dyn_aid"].Expires = TimeStamps.UTCTime().AddDays(-10);
    ...
    public void BindMenuLink()
    {
        try
        {
            ...
            if (CreateUser.CheckIfAdmin(ConDYN, Request.Cookies["dyn_aid"].Value) == true)
            {
                strDashLink = "dashboard.aspx";
                List<UserAccess> ua = CreateUser.LinkIfAdmin(ConDYN).OrderBy(x => x.GroupOrder).ToList();
            ...
            {
                List<UserAccess> ua = CreateUser.GetAllUserAccess(ConDYN, Request.Cookies["dyn_aid"].Value).OrderBy(x => x.GroupOrder).ToList();
