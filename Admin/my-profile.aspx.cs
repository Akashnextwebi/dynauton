...
public partial class Admin_my_profile : System.Web.UI.Page
{
    ...
    public void GetProfileDetails()
    {
        try
        {
            List<CreateUser> comps = CreateUser.GetAllUser(ConDYN).Where(x => x.UserGuid == Request.Cookies["dyn_aid"].Value).ToList();
            if (comps.Count > 0)
            {
                btnSave.Text = "Update";
        ...
        try
        {
            if (Page.IsValid)
            {
                ...
                if (CreateUser.CheckAccess(ConDYN, pageName, "Edit", Request.Cookies["dyn_aid"].Value))
                {
                    ...
                    comp.UserGuid = Request.Cookies["dyn_aid"].Value;
                    comp.AddedBy = Request.Cookies["dyn_aid"].Value;
                    ...
                    comp.Status = "Active";
                    int result = CreateUser.UpdateUserByUser(ConDYN, comp);
                    if (result > 0)
                    {
                        GetProfileDetails();
