...
public partial class Admin_add_blog : System.Web.UI.Page
{
    ...
    public void GetBlogs()
    {
        try
        {
            BlogDetails BD = BlogDetails.GetAllBlogDetailsWithId(ConDYN, Convert.ToInt32(Request.QueryString["id"])).FirstOrDefault();
            if (BD != null)
            {
                divimg.Visible = true;
                ...
                txtTags.Text = BD.Category;
                txtPageTitle.Text = BD.PageTitle;
                txtPostedBy.Text = BD.PostedBy;
                txtPostedOn.Text = Convert.ToDateTime(BD.PostedOn).ToString("dd MMM yyyy");
                if (BD.ThumbImage != "")
                {
            ...
            if (Page.IsValid)
            {
                ...
                if (BD.ThumbImage != "" && BD.BlogImage != "")
                {
                    if (btnSave.Text == "Update")
                    {
                        ...
                        int result = BlogDetails.UpdateBlogDetails(ConDYN, BD);
                        if (result > 0)
                        {
                            GetBlogs();
                    ...
                    {
                        int result = BlogDetails.InsertBlogDetails(ConDYN, BD);
                        if (result > 0)
                        {
        ...
        if (Thumbimage.HasFile)
        {
            try
            {
                ...
                if ((fileExtension == ".jpg" || fileExtension == ".jpeg" || fileExtension == ".png" || fileExtension == ".gif" || fileExtension == ".webp"))
                {
                    ...
                    if ((bitimg.PhysicalDimension.Height != 350) || (bitimg.PhysicalDimension.Width != 550))
                    {
                        return "Size";
                    }
                }
        ...
        if (BlogImage.HasFile)
        {
            try
            {
                ...
                if ((fileExtension == ".jpg" || fileExtension == ".jpeg" || fileExtension == ".png" || fileExtension == ".gif" || fileExtension == ".webp"))
                {
                    ...
                    if ((bitimg.PhysicalDimension.Height != 720) || (bitimg.PhysicalDimension.Width != 1440))
                    {
                        return "Size";
                    }
                }
